...
void mpu6050_initialize()
{
    ...
    Wire.write(BITS_DLPF_CFG_188HZ);//98 BITS_DLPF_CFG_188HZ, BITS_DLPF_CFG_42HZ, default DLPF_CFG = 0 => ACC bandwidth = 260Hz  GYRO bandwidth = 256Hz)
    Wire.endTransmission();  
    delay(5);   
    Wire.beginTransmission(MPU6050_ADDRESS);
    ...
    Wire.write(0x00);//0x02 I2C_BYPASS_EN=1
    Wire.endTransmission();     
}
...
void mpu6050_Gyro_Values()
{
  ...
  Wire.endTransmission();
    gyroRaw[ZAXIS] = ((result[0] << 8) | result[1]);//-12     -3
    gyroRaw[YAXIS] = ((result[2] << 8) | result[3]);//37      15
    gyroRaw[XAXIS] = ((result[4] << 8) | result[5])*-1;//11    5
}	
void mpu6050_Accel_Values()
{
  ...
  Wire.endTransmission();
    accelRaw[ZAXIS] = ((result[0] << 8) | result[1])*-1 + 100;//+ 105 + 100 max=4054.46, min=-4149.48
    accelRaw[YAXIS] = ((result[2] << 8) | result[3])*-1 + 6220;// - 45 - 35 max=4129.57, min=-4070.64
    accelRaw[XAXIS] = ((result[4] << 8) | result[5]) - 500;// + 150 + 170 max=4014.73 , min=-4165.13
...
void sensor_Calibrate()
{
  Serial.print("Sensor_Calibrate");Serial.println("\t");
    ...
    acc_offsetX = 0.18;//0.45 -0.36  Trim PITCH CONTROL
    acc_offsetY = 0.0;//0.16 -0.14 0.18 Trim ROLL CONTROL
